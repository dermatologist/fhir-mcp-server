...
class TestIntegration:
    ...
    def test_fhir_oauth_config_integration(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            callback_url = server_config.callback_url(
                server_config.effective_server_url
            )
    ...
    async def test_oauth_server_provider_integration(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            server_config = ServerConfigs(
                mcp_host="localhost", mcp_port=8080, _env_file=None
            )
            ...
            with patch.object(
                server_provider, "_discover_oauth_metadata"
            ) as mock_discover:
                mock_metadata = Mock()
                ...
                assert (
                    await server_provider._get_authorization_endpoint()
                    == "https://example.com/auth"
                )
                assert (
                    await server_provider._get_token_endpoint()
    ...
    def test_config_url_generation_integration(self):
        ...
        with patch.dict(
        ...
        ):  # Clear env vars to avoid external config
            server_config = ServerConfigs(
            ...
            oauth_callback = server_config.callback_url(
                server_config.effective_server_url
            )
            ...
            assert (
                server_config.discovery_url
                == f"{mock_base_url}/.well-known/smart-configuration"
            )

            # Test FHIR metadata URL with mocked config
            assert (
                server_config.metadata_url == f"{mock_base_url}/metadata?_format=json"
            )
